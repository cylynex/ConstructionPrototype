...
namespace Engine {
    ...
    public class Load : MonoBehaviour {
        ...
        */
        public void LoadCurrentProject(string projectName) {
            print("Load Project Called");
            string pathToLoad = Application.persistentDataPath + "/" + projectName+".dat";
            if (!File.Exists(pathToLoad)) {
                Debug.Log("No saved profile found! Create new project file.  Do stuff that hasnt been done yet here.");
                print("Looking at " + pathToLoad);
                ...
                Debug.Log("SAVE GAME FOUND - Loading: " + pathToLoad);
                BinaryFormatter bf = new BinaryFormatter();
                FileStream fs = File.Open(pathToLoad, FileMode.Open);
                SavedProfile loadedProfile = bf.Deserialize(fs) as SavedProfile;
                fs.Close();

                gameController.ImportProjectSaveData(loadedProfile);


                // Set the name - temp test
                //this.GetComponent<GameController>().characterName = loadedProfile.characterName;
            }
        }
